<template>
  <div id="app">
      ...
      <div class="card">
        ...
        <form @submit.prevent="isEditing ? updateMovie() : addMovie()">
          ...
          <div v-if="Object.values(validationErrors).some(errors => errors.length > 0)" 
            ...
            <ul>
              <template v-for="errors in validationErrors"  :key="error">
    ...
    <div class="sort-controls">
      ...
      <!-- Liste des films -->
        <div  class="deleteFav">
          <button @click="clearAllFavorites" class="btn btn-secondary">
            🗑️ Effacer les favory
          </button>
       </div>
...
import {useValidateForm} from './helpers/ValidateForm'
...
export default {
  ...
  const {validationErrors,validateTitle,validateCategory,validateDescription,validateYear,validateForm}=useValidateForm(newMovie,movies)
  ...
  const clearAllFavorites=()=>{movies.value.forEach(movie => {movie.isFavorite=false});
    showMessage(`"${movie.title}" retiré des favoris`)
  }
  const clearSort = () => {
    sortBy.value = ''
